...
locals {
  linux_user_data_template_file = var.user_data_template_file != "" ? var.user_data_template_file : "linux_user_data.tpl"
  int_linux_default_user_data = var.create && var.platform == "linux" && (var.enable_bootstrap_user_data || var.user_data_template_path != "") ? base64encode(templatefile(
    coalesce(var.user_data_template_path, "${path.module}/../../templates/${local.linux_user_data_template_file}"),
    {
